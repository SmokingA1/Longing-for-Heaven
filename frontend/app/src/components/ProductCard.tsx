...
interface ProductCardProps {
    ...
    onAddToCart: (id: string, size_id: string ) => void;
    ...
    return(
        <div key={product.id} className={`flex flex-col  h-[550px] bg-white py-10 px-4 sm:w-[320px] items-center relative rounded-md hover:z-20 overflow-visible ${hover && "md:hover:shadow-[0_2px_10px_rgba(0,0,0,0.25)]"}`}   onMouseLeave={() => setHover(false)} >
            <div className="flex flex-col gap-3 group" onMouseEnter={() => setHover(true)}>
                <img src={`http://localhost:8000/${product.images[0].photo_url}`} alt="" className="cursor-pointers h-[300px] min-w-[268px]" onClick={() => navigate(`/shop/${product.id}`)}/>
                ...
                <button disabled={productIdsInCart.has(`${product.id}-${selectedSizeID}`)} className={`mb-5 relative w-full p-2.5 rounded-xs duration-150 bg-slate-300 ${productIdsInCart.has(`${product.id}-${selectedSizeID}`) ? "" : "hover:bg-slate-300/70 cursor-pointer" }  mt-5 ${hover ? "block" : "md:hidden"} transition-all`} onClick={() => onAddToCart(product.id, selectedSizeID)}>
