        ...
        <main className="w-[340px] sm:w-150 lg:w-250 xl:w-300 flex-grow py-20 flex justify-center">
            <article className="h-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 grid-rows-[500px] md:grid-rows-[450px] text-sm">
                {products && products.map((product) => (
                    <div key={product.id} className={`flex flex-col h-[500px] w-[300px] bg-white py-10 px-4 items-center  p-2.5 relative rounded-md hover:z-20 overflow-visible ${product.id === hoverEl && " md:hover:shadow-[0_2px_10px_rgba(0,0,0,0.25)]"}`}   onMouseLeave={() => setHoverEl(null)} >
                        <div className="flex flex-col gap-3 group" onMouseEnter={() => setHoverEl(product.id)}>
                            <img src={`http://localhost:8000/${product.images[0].photo_url}`} alt={product.name} className="cursor-pointer h-[300px] w-[268px] " onClick={() => navigate(`/shop/${product.id}`)}/>
                            ...
                            <button className={`relative w-full p-2 py-2.5 rounded-sm duration-150 bg-slate-300 hover:bg-slate-300/70 mt-5 ${product.id === hoverEl ? "block" : "block md:hidden"} transition-all cursor-pointer`} onClick={()=>console.log("added to cart")}>
                                Add to cart
                            </button>
                        </div>
