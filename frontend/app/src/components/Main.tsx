...
const Main: React.FC = () => {
    ...
    return(
        <main className="w-300 flex-grow py-20">
                ...
                {products && products.map((product) => (
                    <div key={product.id} className={`flex flex-col h-[500px] bg-white py-10 px-4 items-center  p-2.5 relative rounded-md hover:z-20 overflow-visible ${product.id === hoverEl && "hover:shadow-[0_2px_10px_rgba(0,0,0,0.25)]"}`}   onMouseLeave={() => setHoverEl(null)} >
                        <div className="flex flex-col gap-5 group" onMouseEnter={() => setHoverEl(product.id)}>
                            <img src={`http://localhost:8000/${product.images[0].photo_url}`} alt={product.name} className="cursor-pointer " onClick={() => navigate(`/shop/${product.id}`)}/>
                            ...
                            <button className={`relative w-full p-2 py-2.5 rounded-sm duration-150 bg-slate-400 hover:bg-slate-400/70 mt-5 ${product.id === hoverEl ? "block" : "hidden"} transition-all cursor-pointer`} onClick={()=>console.log("added to cart")}>
                                Add to cart
                            </button>
                        </div>
